    ...
    def load_clients(self):
        ...
        for cid, name, phone in clients:
            ...
            item = TwoLineAvatarIconListItem(
                text=name,
                secondary_text= الرصيد: %.2f  % balance,
            )
...
class ClientScreen(MDScreen):
    ...
    def load_transactions(self):
        ...
        self.balance_label.text =  %s - الرصيد: %.2f  % (self.client_name, balance)
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        ...
        for tid, ttype, amount, note, date in txs:
            ...
            item = TwoLineAvatarIconListItem(
                text= %s: %s%.2f  % (label, sign, amount),
                secondary_text= %s - %s  % (date, note or   ),
            )
...
class DebtApp(MDApp):
    ...
    def show_add_client(self):
        ...
        phone_field = MDTextField(hint_text= رقم الهاتف , mode= rectangle )
    ...
    def show_add_transaction(self, ttype):
        ...
        note_field = MDTextField(hint_text= ملاحظة , mode= rectangle )
